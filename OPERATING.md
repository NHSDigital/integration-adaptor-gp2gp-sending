...
|-----------------------------------|------------|---------------------------------------------------------------------------------------------------------------------------|
| GP2GP_SERVER_PORT                 | 8080       | The port on which the GP2GP Adaptor API will run.                                                                              |
...
| Environment Variable            | Default                   | Description                                                                                                                                                    |
|---------------------------------|---------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| GP2GP_MONGO_URI                 | mongodb://localhost:27017 | Whole Mongo database connection string. Takes priority over other Mongo variables.                                                                             |
...
| GP2GP_AMQP_MAX_REDELIVERIES           | 3                     | The number of times a message will be retried to be delivered to consumer. After exhausting all retries, it will be put on DLQ.<queue_name> dead letter queue                                                    |
...
[GP Connect API](https://simplifier.net/gpconnect/).
...
Backoff options are used to perform retries of an HTTP request when the client experiences a timeout or 5xx response.
...
* [TaskDefinition.java](service/src/main/java/uk/nhs/adaptors/gp2gp/common/task/TaskDefinition.java)
* [DocumentTaskDefinition.java](service/src/main/java/uk/nhs/adaptors/gp2gp/ehr/DocumentTaskDefinition.java) 
  * [Get GPC Document Task Example](/documentation/examples/Task_queue_payloads/GetGpcDocumentTaskDefinition.md)
* [SendEhrExtractCoreTaskDefinition.java](service/src/main/java/uk/nhs/adaptors/gp2gp/ehr/SendEhrExtractCoreTaskDefinition.java) 
  * [Send EHR Extract Core Task Example](/documentation/examples/Task_queue_payloads/SendEhrExtractCoreTaskDefinition.md)
* [GetGpcStructuredTaskDefinition.java](service/src/main/java/uk/nhs/adaptors/gp2gp/gpc/GetGpcStructuredTaskDefinition.java) 
  * [Get GPC Structured Record Task Example](/documentation/examples/Task_queue_payloads/GetGpcStructuredTaskDefinition.md)
* [SendAcknowledgementTaskDefinition.java](service/src/main/java/uk/nhs/adaptors/gp2gp/ehr/SendAcknowledgementTaskDefinition.java) 
...
* ConversationId: The identifier correlating all GP2GP messages for a single patient transfer
...
- Data stored:
  - The patient's transformed structured record once it has been transformed into an ehrExtract that is ready to send to MHS Outbound
  - Attachments associated with the patient's structured record once they have been transformed into an outbound message for sending to MHS Outbound
- Filename convention:
  - EhrExtract files are named as a concatenation of {conversationId}/{conversationId}.json
...
* The GP2GP adaptor must be used with a MongoDB-compatible database such as Amazon Document DB or Azure Cosmos.
...
long-term data retention. The adaptor also does NOT fulfill HSCIC-PC-BLD-0068.26 requirements 
...
We release the adaptor image on Dockerhub as [nhsdev/nia-gp2gp-adaptor][docker-hub-image],
...
have identified that a minimum of 8GB of RAM and 2 vCPUs allocated to the container is required.
